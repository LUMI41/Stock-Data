        ...
        for index, ticker in enumerate(self.tickers):
            single_EOD = np.genfromtxt(
                os.path.join(self.data_path, self.market_name + '_' + ticker +
                             '.T_30Y.csv'), dtype=str, delimiter=',',
                skip_header=True
            )
            self.data_EOD.append(single_EOD)
            # if index > 99:
            #     break
        print('#stocks\' EOD data reading:', len(self.data_EOD))
    ...
    def generate_feature(self, selected_tickers_fname, begin_date, opath,
        ...
        trading_dates = np.genfromtxt(
            os.path.join(self.data_path, '..',
                         self.market_name + '_aver_line_dates_2.csv'),
        ...
        #     dtype=str, delimiter='\t', skip_header=False
        # )
        tickers = pd.read_csv(selected_tickers_fname)
        self.tickers = tickers['Local Code'].astype(str).tolist()
...
if __name__ == '__main__':
    ...
    current_dir = f"/Users/kaveeshathalallehewage/PycharmProjects/stock selection/"

    if args.path is None:
        args.path = os.path.join(current_dir,f"data/TSE_Stock_Data")
    if args.market is None:
        args.market = 'TSE'
    #read tickers
    tickers = f"../wiki/tickers_tse_qualified_prime.csv"
    # tickers = tickers['Local Code'].tolist()

    processor = EOD_Preprocessor(args.path, args.market)
    processor.generate_feature(
        tickers,
        datetime.strptime('2012-11-18', processor.date_format),
        os.path.join(processor.data_path, '..', 'processed_TSE_wo_normalizing'), return_days=1,
        pad_begin=29
    )
